...
namespace Interpreter
{
					...
					u32 load, u32 reg_n, u32 reglist)
	{
		...
		if(writeback){Assert(((1 << reg_n) & reglist) == 0);} //mostly invalid
		...
		while(true)
		{
			if(((1 << i) & reglist) != 0)
			{
				...
				else
				{
					...
					if(i != 15){setGPR(i, memval);}
					else{setPC(memval);}
				}
				if(!before){addr += increment;}
			}
			i += (up ? 1 : -1);
			if((up && i == 16) || (!up && i == -1)){break;}
		}
		if(writeback){setGPR( reg_n, addr );}
	}
}