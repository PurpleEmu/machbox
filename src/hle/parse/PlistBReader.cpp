...
namespace Plist
{
	BReader::BReader(const vector<u8>& data)
	: m_data(data)
	{
		ReadHeader();
		ReadFooter();
	...
	Node* BReader::ReadNode(int nodeAddr) //unsafe
	{
		...
		{
			case 0: //null, bool
				if(len == 0){return new NullNode();}
				else if(len == 8){return new BoolNode(false);}
				else if(len == 9){return new BoolNode(true);}
				else{Fail(type);}
			case 1: //int
				return new IntNode(Read(nodeAddr + 1, (1 << len)));
			case 2: //real
				if(len == 2){return new RealNode(As<float,u32>(Read32(nodeAddr + 1)));} //float
				else if(len == 3){return new RealNode(As<double,u64>(Read64(nodeAddr + 1)));} //double
				else{Fail(type);}
			case 3: //date
				if(len == 3){return new DateNode(As<double,u64>(Read64(nodeAddr + 1)));}
				else{Fail(type);}		
			case 4: //data
				{
					...
					if(!data){Fail(0);}
					for(u32 i = 0; i < size; i++){data[i] = Read8(dataAddr + i);}
			...
			case 5: //ascii string
				{
					...
					wstring wstr(size, '\0');
					for(u32 i = 0; i < size; i++){wstr[i] = Read8(strAddr + i);}
			...
			case 6: //wide string
				{
					...
					int strAddr = GetDataAddr(len, nodeAddr + 1);
					wstring wstr(size, '\0');
					for(u32 i = 0; i < size; i++){wstr[i] = Read8(strAddr + i*2);}
					return new StringNode(wstr);
				}
			case 8: //uid
			...
			case 12: //set
				{
					...
					if(typehigh == 10){return new ArrayNode(vec);}//array
					else{return new SetNode(vec);}//set	
				}
			case 13: //dict
				{
					u32 size = (u32)ReadInt(len, nodeAddr + 1);
					int mapAddr = GetDataAddr(len, nodeAddr + 1);
					vector<Node*> *vec2 = new vector<Node*>(size*2);
					if(!vec2){Fail(0);}
					for(u32 i = 0; i < size; i++)
					{
	...
	{
		if(len != 15){return nodeAddr;}
		else
		{
			u8 type = Read8(nodeAddr);
			if((type >> 4) != 1){Fail(type);}
	...
	u64 BReader::Read(int addr, int size)
	{
		...
		else if(size == 2){return Read16(addr);}
		else if(size == 4){return Read32(addr);}
		else if(size == 8){return Read64(addr);}
		else{Fail(size);}
	}
}